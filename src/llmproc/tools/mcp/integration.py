...
async def initialize_mcp_tools(
    ...
    if not manager:
        manager = MCPManager(
            config_path=config_path, tools_config=tools_config, provider=provider
        )
    ...
    try:
        ...
        if not success:
            ...
            raise RuntimeError(
                "Failed to initialize MCP tools. Some tools may not be available."
            )
        ...
        if len(tool_names) == 0 and tools_config:
            logger.warning(
                f"No MCP tools were registered despite having MCP configuration: {tools_config}"
            )
...
def register_mcp_tool(
    mcp_registry: ToolRegistry, runtime_registry: ToolRegistry, tool_name: str
) -> bool:
...
def register_runtime_mcp_tools(
    mcp_registry: ToolRegistry, runtime_registry: ToolRegistry, enabled_tools: list[str]
) -> int:
