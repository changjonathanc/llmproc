...
def prepare_tool_handler(func: Callable) -> Callable:
    ...
    # Create handler function with explicit parameters only
    async def handler(**kwargs) -> ToolResult:
        ...
        try:
            ...
            if is_async:
                # Call async function
                result = await func(**function_kwargs)
            else:
                # Call sync function
                result = func(**function_kwargs)

            # Return success result
            result_obj = ToolResult(content=result, is_error=False)
            # Debug line removed
            return result_obj
...
def create_process_aware_handler(func: Callable, process: Any) -> Callable:
    ...
    # Create a handler with explicit parameters only
    async def handler(**kwargs) -> Any:
        # Gather function kwargs from the provided parameters
        function_kwargs = {}
