...
class ToolManager:
    ...
    - Phase 2 (Function Tool Migration): Complete
    - Phase 3 (System Tool Integration): Complete
    - Phase 4 (Tool Definition Standardization): Complete
    - Phase 5 (Final Integration and Cleanup): Complete
    """
        ...
        for tool_name in enabled_tools:
            if tool_name in tool_registrars:
                ...
                if tool_name == "spawn" and not getattr(process, "has_linked_programs", False):
                    logger.debug(f"Skipping spawn tool registration: no linked programs")
                    continue
    ...
    def _register_function_based_tool(self, process, tool_name):
        ...
        # Import the appropriate function based on the tool name
        if tool_name == "calculator":
            from llmproc.tools.calculator import calculator as func
        elif tool_name == "read_file":
            from llmproc.tools.read_file import read_file as func
        else:
    ...
    def _register_spawn_tool(self, process, tool_name):
        ...
        if fd_enabled:
            ...
            spawn_def = SPAWN_TOOL_SCHEMA_WITH_FD.copy()
        else:
            from llmproc.tools.spawn import SPAWN_TOOL_SCHEMA_BASE
            spawn_def = SPAWN_TOOL_SCHEMA_BASE.copy()
        ...
        # Import tool function and create process-aware handler
        from llmproc.tools.spawn import spawn_tool
        from llmproc.tools.function_tools import create_process_aware_handler
    ...
    def _register_fd_tools(self, process, register_read_fd, register_fd_to_file):
        ...
        """
        from llmproc.tools.file_descriptor import read_fd_tool_def, fd_to_file_tool_def
        from llmproc.tools.function_tools import create_process_aware_handler
