...
class ToolManager:
    ...
    def __init__(self):
        ...
        self.builtin_registry = (
            ToolRegistry()
        )  # For storing all available builtin tools
    ...
    def set_enabled_tools(self, tools_config: list[Union[str, Callable]]):
        ...
        if not isinstance(tools_config, list):
            raise ValueError(
                f"Expected a list of tool names/callables, got {type(tools_config)}"
            )
        ...
        logger.info(
            f"ToolManager: Tools marked as enabled: {self.enabled_tools} (note: actual registration occurs during initialization)"
        )
    ...
    def process_function_tools(self):
        ...
        logger.info(
            f"REGISTRATION PHASE: Processing {len(self.function_tools)} function tools based on enabled list: {self.enabled_tools}"
        )
            ...
            try:
                ...
                if tool_name in self.enabled_tools:
                    ...
                    if tool_name not in self.runtime_registry.tool_handlers:
                        ...
                        logger.debug(
                            f"Skipping registration of function tool '{tool_name}' as it's already registered (possibly as a system tool)."
                        )
                else:
                    logger.debug(
                        f"Function tool '{tool_name}' is not in the enabled list, skipping registration"
                    )

            except Exception as e:
                logger.error(
                    f"Error processing function tool {func_tool.__name__}: {str(e)}"
                )

        logger.info(
            f"REGISTRATION COMPLETE: Processed {len(self.function_tools)} function tools. Registered {registered_count} enabled tools."
        )
        return self
    ...
    def get_tool_schemas(self) -> list[dict[str, Any]]:
        ...
        if reverse_aliases:
            filtered_schemas = apply_aliases_to_schemas(
                filtered_schemas, reverse_aliases
            )
    ...
    def register_system_tools(self, config: dict[str, Any]) -> "ToolManager":
        ...
        """
        logger.info(
            f"REGISTRATION PHASE: Starting system tools registration based on enabled list: {self.enabled_tools}"
        )

        # Delegate to the integration module
        registered_count = register_system_tools(
            self.builtin_registry, self.runtime_registry, self.enabled_tools, config
        )
    ...
    def _load_builtin_tools(self):
        ...
        if success:
            ...
            logger.info(
                f"Loaded {len(self.builtin_registry.tool_handlers)} builtin tools into builtin registry"
            )
        else:
    ...
    async def initialize_tools(self, config: dict[str, Any]) -> "ToolManager":
        ...
        """
        logger.info(
            f"Starting tool initialization for {len(self.enabled_tools)} enabled tools"
        )
        ...
        if mcp_enabled and mcp_config_path:
            logger.info(
                "MCP tools are enabled, attempting initialization with configuration"
            )
            # Use the extracted MCP integration functions
            # Use updated parameter name for tool_registry (previously mcp_registry)
            success, manager = await initialize_mcp_tools(
                config, self.mcp_registry, self.mcp_manager
            )
            if success:
                ...
                register_runtime_mcp_tools(
                    self.mcp_registry, self.runtime_registry, self.enabled_tools
                )
    ...
    def setup_runtime_context(self, process: Any) -> "ToolManager":
        ...
        if (
            hasattr(process, "linked_program_descriptions")
            and process.linked_program_descriptions
        ):
            runtime_context["linked_program_descriptions"] = (
        ...
        logger.debug(
            f"Runtime context set up with keys: {', '.join(runtime_context.keys())}"
        )
        return self