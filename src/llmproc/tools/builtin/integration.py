...
def register_system_tools(
    ...
    tool_registrars = {
        # Basic tools
        "calculator": lambda name: copy_tool_from_source_to_target(source_registry, target_registry, name),
        "read_file": lambda name: copy_tool_from_source_to_target(source_registry, target_registry, name), 
        "list_dir": lambda name: copy_tool_from_source_to_target(source_registry, target_registry, name),
        ...
        "spawn": lambda name: register_spawn_tool(source_registry, target_registry, name, linked_programs, linked_program_descriptions),
        "read_fd": lambda name: register_fd_tool(source_registry, target_registry, name, fd_manager),
        "fd_to_file": lambda name: register_fd_tool(source_registry, target_registry, name, fd_manager),
    }
    ...
    for tool_name in enabled_tools:
        if tool_name in tool_registrars:
            ...
            if tool_name == "spawn" and not has_linked_programs:
                logger.info(f"Skipping spawn tool registration: marked as enabled but no linked programs available")
                continue
...
def register_fd_tool(
    ...
    if not fd_manager:
        logger.warning(f"Cannot register {tool_name} tool: No file descriptor manager provided")
        return False
