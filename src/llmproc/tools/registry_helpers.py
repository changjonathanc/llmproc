...
def extract_tool_components(
    ...
    definition = next((d for d in registry.get_definitions() 
                     if d.get("name") == tool_name), None)
    if not definition:
...
def check_for_duplicate_schema_names(schemas: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i, schema in enumerate(schemas):
        ...
        if name in seen_names:
            logger.warning(
                f"Duplicate tool name '{name}' found at indices {seen_names[name]} and {i}. "
