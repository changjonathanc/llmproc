...
class ProgramLoader:
    ...
    def _build_from_config(
        ...
        linked_programs, linked_program_descriptions = (
        ...
        program = LLMProgram(
            ...
            file_descriptor=config.file_descriptor.model_dump()
            if config.file_descriptor
            else None,
            base_dir=base_dir,
        ...
        for file_path in config.preload.files:
            try:
                ...
                if not resolved_path.exists():
                    warnings.warn(
                        f"Preload file not found: {resolved_path}", stacklevel=2
                    )
                preload_files.append(str(resolved_path))
            except Exception as e:
                warnings.warn(
                    f"Error resolving path '{file_path}': {str(e)}", stacklevel=2
                )
