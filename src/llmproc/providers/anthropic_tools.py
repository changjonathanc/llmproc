...
async def run_anthropic_with_tools(
    ...
    **kwargs
    ...
    while iterations < max_iterations:
        ...
        try:
            ...
            try:
                ...
                dump_file = dump_api_error(e, "anthropic", model_name, system_prompt, 
                                          messages, temperature, max_tokens, tools, iterations)
                raise RuntimeError(f"Error calling Anthropic API: {str(e)} (see {dump_file} for details)")
    ...
    if debug:
        print(f"=== Anthropic Tool Execution Complete ({iterations}/{max_iterations} iterations) ===\n")
...
def filter_empty_text_blocks(messages: List[Dict[str, Any]], debug: bool = False) -> List[Dict[str, Any]]:
