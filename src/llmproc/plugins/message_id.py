...
class MessageIDPlugin:
    """Plugin that prefixes message IDs and optionally exposes the goto tool."""

    def __init__(self, config: MessageIDPluginConfig | None = None) -> None:
        config = config or MessageIDPluginConfig()
        self.config = config
        self.enable_goto = config.enable_goto

    def fork(self) -> MessageIDPlugin:
        """Return ``self`` for a forked process."""
        return self

    async def hook_user_input(self, user_input: str, process) -> str | None:
        """Prefix user input with a message ID."""
        msg_id = len(process.state)
        return f"[msg_{msg_id}] {user_input}"
    ...
    async def goto_tool(self, position: str, message: str, runtime_context: Optional[dict[str, Any]] = None):
        ...
        if message:
            ...
            original_message = (
                f"<original_message_to_be_ignored>\n{original_content}\n</original_message_to_be_ignored>"
            )
            ...
            # Use append_message to ensure it gets a proper ID
            append_message(process, "user", final_message)
    ...
    def hook_provide_tools(self) -> list:
        """Return the goto tool if enabled."""
        if not self.enable_goto:
            return []

        return [self.goto_tool]
