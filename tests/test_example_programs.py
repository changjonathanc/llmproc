    ...
    for program_path in example_programs:
        ...
        with open(full_path, "rb") as f:
            try:
                ...
                has_model_info = (
                    ("model_name" in program and "provider" in program) or
...
async def test_minimal_functionality():
    ...
    assert "2" in response and "+" in response, "Expected model to remember previous question"
    
    # Test reset functionality
    process.reset_state()
    response = await process.run("What was my previous question?")
    assert "previous question" not in response.lower() or "don't" in response.lower() or "no" in response.lower(), \
...
async def test_program_linking_functionality():
    ...
    assert "llm" in response.lower() and ("process" in response.lower() or "api" in response.lower()), \
    ...
    for message in state:
        if message.get("role") == "assistant" and message.get("content"):
            if "spawn" in message.get("content").lower() and "repo_expert" in message.get("content").lower():
...
async def test_file_preload_functionality():
    ...
    assert any(term in response.lower() for term in ["api", "llm", "process", "interface"]), \
...
async def test_claude_code_comprehensive():
    ...
    assert any(term in preload_response.lower() for term in ["llm_process", "providers", "tools"]), \
    ...
    assert any(term in combined_response.lower() for term in 
               ["anthropic", "openai", "vertex", "claude", "gpt", "example"]), \
...
async def test_provider_specific_functionality():
    ...
    for program_name, expected_terms in provider_programs.items():
        ...
        if program_name == "vertex.toml" and "GOOGLE_APPLICATION_CREDENTIALS" not in os.environ:
            continue
...
def run_cli_with_input(program_path: Path, input_text: str, timeout: Optional[int] = 30) -> str:
...
@pytest.mark.parametrize("program_name", [
    ...
    "mcp.toml",
    "preload.toml",
    pytest.param("vertex.toml", marks=pytest.mark.skipif(
...
def test_cli_with_all_programs(program_name):
    ...
    try:
        ...
        assert "Configuration" in output, f"Expected CLI using {program_name} to show program information"
