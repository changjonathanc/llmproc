...
def test_goto_context_compaction():
    ...
    try:
        ...
        assert result.returncode == 0, f"Demo script failed with return code {result.returncode}. Error: {result.stderr}"
    ...
    if not token_reduction_match:
        ...
        if percent_match:
            ...
            assert False, "Could not find token reduction information in script output"
    else:
