...
and runs a simple conversation with tool usage using the async run method.
"""
...
async def run_examples(llm_process):
    """Run examples using the LLMProcess with MCP tools."""
    ...
    if "github" in llm_process.mcp_tools:
        response = await llm_process.run(
...
async def main():
    ...
    await run_examples(llm_process)
